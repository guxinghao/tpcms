...
$(function () {
    ...
    if(device_type == 1)
    {
        $('.header-title h1').text('出售二手设备');
    }else if(device_type == 2)
    {
        $('.header-title h1').text('出租二手设备');
    }else if(device_type == 3)
    {
        $('.form-price').hide();
        $('.header-title h1').text('求购二手设备');
    }else if(device_type == 4)
    {
        $('.form-price').hide();
        $('.header-title h1').text('求租二手设备');
    }
...
$('#addFactory').click(function(){
    ...
    var result = {};
    result.device_name = $('#device_name').val();
    result.contacts = $('#contacts').val();
    result.mobile = $('#mobile').val();
    result.device_content = $('#device_content').val();
    ...
    $.ajax({
        type: "post",
        url: ApiUrl + "/index.php?ctl=Ve_Action&met=adddevice&typ=json",
